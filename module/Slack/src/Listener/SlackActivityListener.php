    ...
    public function handleEvent(Event $event)
    {
		...
		switch($event->getName())
		{
			...
			case "task.review":
				...
				$logger->info("Slack: event data => " . $eventParams);
				break;
			case "task.comment":
				try {
				...
				break;
			default:
				$logger->info("Slack: event not treated " . $eventString);
				return;
		}
		...
		$eventString = str_replace('localhost', $config['slack']['swarm_host'], $eventString);
		
        $logger->info("Slack: " . $eventString);

		// URL to POST messages to Slack
		$slackUrl = $config['slack']['slack_webhook'];
        $this->postSlack($slackUrl, $eventString);
